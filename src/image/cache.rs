...
pub fn load(image_path: &Path, backend: &str, width: u16, height: u16) -> Option<CachedRender> {
    ...
    if cached_mtime != file_mtime(image_path)? { return None; } // stale
...
pub fn save(image_path: &Path, backend: &str, width: u16, height: u16,
            data: &str, cols: u16, rows: u16) {
    let Some(mtime) = file_mtime(image_path) else { return };
