...
impl InfoModule for BatteryModule {
    ...
    fn value(&self) -> anyhow::Result<String> {
        if let Some((pct, status)) = get_battery_info(self.timeout) {
...
#[cfg(target_os = "linux")]
fn get_battery_info(_timeout: Duration) -> Option<(u8, String)> {
...
#[cfg(target_os = "macos")]
fn get_battery_info(timeout: Duration) -> Option<(u8, String)> {
