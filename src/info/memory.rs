...
#[cfg(target_os = "macos")]
use super::{run_command_stdout, run_command_timeout};
...
impl InfoModule for MemoryModule {
    ...
    fn value(&self) -> anyhow::Result<String> {
        let (used, total) = read_mem(self.timeout)?;
        if total == 0 {
...
#[cfg(target_os = "linux")]
fn read_mem(_timeout: Duration) -> anyhow::Result<(u64, u64)> {
    let m = procfs::read_meminfo()?;
    Ok((m.mem_used(), m.mem_total))
}

#[cfg(target_os = "macos")]
fn read_mem(timeout: Duration) -> anyhow::Result<(u64, u64)> {
    let total = run_command_stdout(
        {
    ...
    let out = run_command_timeout(std::process::Command::new("vm_stat"), timeout)
        .ok_or_else(|| anyhow::anyhow!("vm_stat failed"))?;
